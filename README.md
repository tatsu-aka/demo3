...
## 入出庫履歴画面
- 商品ごとの入庫・出庫履歴を一覧表示する画面です。
- 履歴データは在庫推移グラフの元データとして利用され、在庫管理の整合性を保つ機能となっています。

![入庫履歴画面](./images/stock-history-in.png)

![出庫履歴画面](./images/stock-history-out.png)
...
## メーカー登録画面（マスタ）
- メーカー（取引先）の登録を行う画面です。
商品登録や在庫内訳表示で使用されるためアプリ全体の基盤となるデータを管理します。  
- 商品登録画面・入庫画面・出庫画面と連動しています。  

![メーカー登録画面](./images/maker-list.png)

---

## 商品登録画面（マスタ）
- 商品名・カテゴリ・メーカー・単位などアプリ全体の基盤となる商品情報を管理する画面です。
- 入庫・出庫・在庫内訳・価格履歴など他機能の中心となるデータを管理。

![商品登録画面](./images/product-master-list.png)
...
![入出庫別グラフ](./images/stock-in-out-graph.png)
...
## ユーザー登録画面
- ユーザー名・パスワード・権限（ADMIN/USER）を登録する画面です・
- ユーザー名の重複チェック・パスワードのハッシュ化・権限の付与を行い、入力チェック・パスワード保護・ロール設計を意識して実装しました。

![ユーザー登録画面](./images/user-new.png)
...
- テストコード（JUnit）によるControllerのテスト追加
 
  MockMvcを用いたControllerの単体テストを追加しREST APIの信頼性を高めたい